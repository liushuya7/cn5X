...
class HandEyeCalibrationDialog(QDialog):
    ...
    def recordData(self):
        ...
        # print("marker_T")
        # print(marker_T_pose)
        print("process:{:.1f}%".format(self.i_cmd/len(self.motion_joints)*100))
    ...
    def automaticCalibration(self):
        ...
        b_vals = np.flip(np.linspace(-720, 0, num=num_poses, endpoint=False))
    ...
    def calibrate(self):
        if len(self.robot_poses) < 4:
            ...
            print(np.concatenate((X[:3,3].T, Rx.as_quat()), axis=0))
